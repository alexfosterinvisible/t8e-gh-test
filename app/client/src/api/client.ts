...
export const api = {
  ...
  async exportQueryResultsAsJson(
    data: any[],
    columns: string[],
  ): Promise<void> {
  ...
  async generateTableData(tableName: string): Promise<DataGenerationResponse> {
    return apiRequest<DataGenerationResponse>("/generate-data", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
