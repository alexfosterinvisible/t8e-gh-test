...
export class AEAPollingService {
  ...
  private hasAgentChanged(agent: AEAAgent): boolean {
    ...
    if (changed) {
      console.debug(
        `[AEA Polling] Agent ${this.agentId} changed - updating UI`,
      );
      this.lastAgentHash = currentHash;
    } else {
      console.debug(
        `[AEA Polling] Agent ${this.agentId} unchanged - skipping UI update`,
      );
    }
  ...
  private adjustPollingInterval(state: string): void {
    ...
    if (newInterval !== this.pollInterval) {
      console.debug(
        `[AEA Polling] Adjusting interval from ${this.pollInterval}ms to ${newInterval}ms (state: ${state})`,
      );
