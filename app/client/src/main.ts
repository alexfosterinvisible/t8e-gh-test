...
let currentOutputMode: "sql" | "abap" = "sql"; // (Claude) Output mode state

// Initialize app
document.addEventListener("DOMContentLoaded", () => {
  initializeQueryInput();
  initializeOutputModeToggle(); // (Claude) Initialize output mode toggle
  ...
  const executeQuery = async () => {
    ...
    try {
      const response = await api.processQuery({
        ...
        output_mode: currentOutputMode, // (Claude) Include output mode
      });
...
// (Claude) Output Mode Toggle Functionality
function initializeOutputModeToggle() {
  const sqlButton = document.getElementById(
    "sql-mode-button",
  ) as HTMLButtonElement;
  const abapButton = document.getElementById(
    "abap-mode-button",
  ) as HTMLButtonElement;

  if (!sqlButton || !abapButton) {
    console.error("Output mode buttons not found");
    return;
  }

  // SQL mode button handler
  sqlButton.addEventListener("click", () => {
    if (currentOutputMode !== "sql") {
      currentOutputMode = "sql";
      sqlButton.classList.add("active");
      abapButton.classList.remove("active");
    }
  });

  // ABAP mode button handler
  abapButton.addEventListener("click", () => {
    if (currentOutputMode !== "abap") {
      currentOutputMode = "abap";
      abapButton.classList.add("active");
  ...
  generateButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (response && typeof response.query === "string") {
        ...
        if (response.query === "") {
          displayError(
            "No query generated - please check if you have uploaded data tables",
          );
        }
      } else {
      ...
      displayError(
        error instanceof Error
          ? error.message
          : "Failed to generate random query",
      );
    } finally {
...
function displayResults(response: QueryResponse, query: string) {
  ...
  // (Claude) Determine if ABAP mode
  const isAbapMode =
    response.output_mode === "abap" || currentOutputMode === "abap";
  const codeLabel = isAbapMode ? "ABAP" : "SQL";
  const codeContent = response.code || response.sql;

  // Display natural language query and generated code
  sqlDisplay.innerHTML = `
    <div class="query-display">
      <strong>Query:</strong> ${query}
    </div>
    <div class="sql-query">
      <strong>${codeLabel}:</strong> <code>${codeContent}</code>
    </div>
  `;

  // Display results table or ABAP message
  if (response.error) {
    resultsContainer.innerHTML = `<div class="error-message">${response.error}</div>`;
  } else if (isAbapMode) {
    // (Claude) ABAP mode - show informational message
    resultsContainer.innerHTML = `
      <div style="padding: 1rem; background: rgba(102, 126, 234, 0.1); border: 1px solid var(--primary-color); border-radius: 8px; color: var(--text-primary);">
        <strong>ABAP code generated</strong><br>
        <small>Execution is not supported (ABAP requires SAP system connectivity)</small>
      </div>
    `;
  ...
  toggleButton.addEventListener("click", () => {
    ...
    toggleButton.textContent =
      resultsContainer.style.display === "none" ? "Show" : "Hide";
  });
  ...
  if (!response.error && response.results.length > 0) {
    ...
    const existingButtonContainer = resultsHeader.querySelector(
...
function displayTables(tables: TableSchema[]) {
  ...
  tables.forEach((table) => {
    ...
    // Create JSON export button
    const jsonExportButton = document.createElement("button");
    jsonExportButton.className =
      "export-button table-export-button json-export";
    jsonExportButton.innerHTML = "🔤 JSON";
...
function displayUploadSuccess(response: FileUploadResponse) {
  ...
  const tablesSection = document.getElementById(
    "tables-section",
  ) as HTMLElement;
...
async function initializeAEASystem() {
  ...
  console.log(
    "AEA Dev Console initialized. Press Cmd+P (Mac) or Ctrl+P (Windows/Linux) to open.",
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (status.running) {
        console.log(
          "AEA Server is running. Checking for existing agent sessions...",
        );

        // Get all active agent IDs
        const agentIdsResponse = await fetch(
          "http://localhost:8743/aea/get_agent_ids",
        );
        if (agentIdsResponse.ok) {
          ...
          if (agentIds.length > 0) {
            console.log(
              `Found ${agentIds.length} active agent sessions. Opening windows...`,
            );
